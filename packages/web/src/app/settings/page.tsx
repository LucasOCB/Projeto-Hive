...
export default function SettingsPage() {
  ...
  const [savingProvider, setSavingProvider] = useState<'gemini' | 'freepik' | null>(null);
  ...
  async function handleSaveProvider(provider: 'gemini' | 'freepik') {
    if (provider === imageProvider) return; // já está activo
              ...
              <div className="mb-4">
                ...
                <div className="flex gap-2">
                  {(['gemini', 'freepik'] as const).map((p) => {
                    const isActive = imageProvider === p;
                    const isSaving = savingProvider === p;
                    return (
                      <button
                        key={p}
                        onClick={() => handleSaveProvider(p)}
                        disabled={savingProvider !== null}
                        className={`flex-1 py-2.5 px-3 rounded-lg text-xs font-semibold border transition-all flex items-center justify-center gap-1.5 ${
                          isActive
                            ? 'bg-primary/15 border-primary text-primary'
                            : 'bg-bg-main border-border text-text-secondary hover:border-primary/50 hover:text-text-primary'
                        }`}
                      >
                        {isSaving ? (
                        ...
                        {isActive && <span className="text-[10px] font-bold uppercase tracking-wide opacity-70">ativo</span>}
                      </button>
                    );
                  })}
                </div>
